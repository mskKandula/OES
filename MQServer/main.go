...
func main() {
	...
	conn, err := amqp.Dial("amqp://rabbitmq:rabbitmq@messageq/")
...
func HlsVideoConversion(result string) {
	...
	cmd := exec.Command("ffmpeg", "-i", result, "-codec:", "copy", "-start_number", "0", "-hls_time", "10", "-hls_list_size", "0", "-f", "hls", dir+"index.m3u8")
